...
abstract class SpanningTreeVisitor extends GraphVisitor {
  Edge getParentEdge(Node node) {
    return node.workingData[1];
  }
  EdgeList getSpanningTreeChildren(Node node) {
    return node.workingData[0];
  }
  Node getTreeHead(Edge edge) {
    if (getParentEdge(edge.source) == edge) return edge.target;
    return edge.source;
  }
  ...
  Node getTreeTail(Edge edge) {
    if (getParentEdge(edge.source) == edge) return edge.source;
    return edge.target;
  }
