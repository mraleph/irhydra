 ...
 * <LI>All edge's must be added to the graph's {@link DirectedGraph#edges edges}list exactly once.
 * <LI>All nodes must be added to the graph's {@link DirectedGraph#nodes nodes}list exactly once.
 * </UL>
 * This algorithm will:
 * <UL>
 ...
 * exact behavior is undefined, but will favor edge's with higher{@link Edge#weight}s. The minimum x value assigned to a node or bendpoint
 * will be 0.
 ...
 * For each EDGE:
 * <UL>
 * <LI>If an edge spans more than 1 row, it will have a list of{@link org.eclipse.draw2d.graph.Edge#vNodes virtual} nodes. The virtual nodes
...
class DirectedGraphLayout {
   ...
   * @param graphthe graph to layout
   */
  void visit(DirectedGraph graph) {
    if (graph.nodes.isEmpty) return;
    for (int i = 0; i < steps.length; i++) {
      GraphVisitor visitor = steps[i];
      visitor.visit(graph);
    }
