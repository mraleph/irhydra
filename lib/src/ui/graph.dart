...
display(pane, blocks, attachRef) {
  ...
  for (var edge in g.edges) {
    ...
    svg.nodes.add(_pathFromPoints(edge.points, color));
  }
...
_createRect({x, y, width, height, r: 0, fill: "white", stroke: "black"}) {
...
_selectFill(value) {
  final idx = math.min(value, BREWER_PALETTE.length) - 1;
  return (value == 0) ? "white" : BREWER_PALETTE[idx];
}
