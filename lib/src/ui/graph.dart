...
const LABEL_STYLE = "font-family: Monaco, Menlo, Consolas, "
                    "\"Courier New\", monospace; font-size: 24pt; font-weight: bold;";
...
_pathFromPoints(graph.Dimension size, points, color) {
  ...
  var v1_x = 20 * math.cos(angle + headAngle);
  var v1_y = 20 * math.sin(angle + headAngle);

  var v2_x = 20 * math.cos(angle - headAngle);
  var v2_y = 20 * math.sin(angle - headAngle);
...
_selectStroke(block) {
  if (block.marks.contains("deoptimizes")) {
    return const _Stroke(color: "#8E44AD", width: "5px");
  } else if (block.marks.contains("changes-all")) {
    return const _Stroke(color: "red", width: "5px", dashArray: "10,5");
  } else {
    return const _Stroke(color: "black", width: "3px");
  }
}
