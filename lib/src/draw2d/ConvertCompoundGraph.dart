...
class ConvertCompoundGraph extends GraphVisitor {
  ...
  void connectHead(CompoundDirectedGraph graph, Node node, Subgraph parent) {
    bool connectHead = true;
    for (int j = 0; connectHead && j < node.incoming.length; j++) {
      Node ancestor = node.incoming[j].source;
      if (parent.isNested(ancestor)) connectHead = false;
    }
    if (connectHead) {
  ...
  void connectTail(CompoundDirectedGraph graph, Node node, Subgraph parent) {
    bool connectTail = true;
    for (int j = 0; connectTail && j < node.outgoing.length; j++) {
      Node ancestor = node.outgoing[j].target;
      if (parent.isNested(ancestor)) connectTail = false;
    }
    if (connectTail) {
  ...
  void convertSubgraphEndpoints(CompoundDirectedGraph graph) {
    for (int i = 0; i < graph.edges.length; i++) {
      ...
      if (edge.source is Subgraph) {
        ...
        Node newSource;
        if (s.isNested(edge.target)) newSource = s.head; else newSource = s.tail;
      ...
      if (edge.target is Subgraph) {
        ...
        Node newTarget;
        if (s.isNested(edge.source)) newTarget = s.tail; else newTarget = s.head;
  ...
  void replaceSubgraphsWithBoundaries(CompoundDirectedGraph graph) {
    for (int i = 0; i < graph.subgraphs.length; i++) {
      Subgraph s = graph.subgraphs.get(i);
  ...
  void visit(DirectedGraph dg) {
    ...
    for (int i = 0; i < graph.nodes.length; i++) {
      ...
      if (node is Subgraph) {
        ...
        if (s.members.length == 2) graph.edges.add(new Edge(s.head, s.tail));
      }
    }
