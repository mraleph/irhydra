...
class LeftRight {
  ...
  bool operator == (LeftRight entry) {
    return (entry.left == left) && (entry.right == right);
  }
...
class CompoundHorizontalPlacement extends HorizontalPlacement {
  ...
  void applyGPrime() {
    super.applyGPrime();
    NodeList subgraphs = (graph as CompoundDirectedGraph).subgraphs;
    for (int i = 0; i < subgraphs.length; i++) {
      Subgraph s = subgraphs.get(i);
      s.x = s.left.x;
